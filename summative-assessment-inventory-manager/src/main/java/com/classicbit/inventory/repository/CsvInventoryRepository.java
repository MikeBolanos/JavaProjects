        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (parts.length == 7) {
                    ...
                    String regionLock = parts[4].trim();
                    int quantity = Integer.parseInt(parts[5].trim());
                    BigDecimal price = new BigDecimal(parts[6].trim());

                    com.classicbit.inventory.model.Console console = new com.classicbit.inventory.model.Console(id, name, manufacturer, releaseYear, regionLock);
                    InventoryItem item = new InventoryItem(console, quantity, price);
                    inventory.put(id, item);
                }
            }
        } catch (IOException e) {
    ...
    private void saveToFile() {
        try (PrintWriter writer = new PrintWriter(new FileWriter(filename))) {
            for (InventoryItem item : inventory.values()) {
                Console console = item.getConsole();
